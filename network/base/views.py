...
def calculate_polar_data(observer, satellite, start, end, points):
     ...
     while temp_date < end:
         ...
         data.append([float(format(math.degrees(satellite.alt), '.4f')),float(format(math.degrees(satellite.az), '.4f'))])
                    ...
                    if tr < ephem.date(datetime.today() + timedelta(hours=6)):
                        if (float(elevation) >= station.horizon and tr < ts):
                            ...
                            if tr < ephem.Date(datetime.now() +
                                ...
                                valid = False
                            polar_data = calculate_polar_data(observer, sat_ephem, tr.datetime(), ts.datetime(), 10)
